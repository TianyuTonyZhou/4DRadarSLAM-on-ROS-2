...
void read_topic_from_rosbag2(const std::string& bag_path,
                             std::vector<sensor_msgs::msg::NavSatFix::SharedPtr>& msgs_queue) {
    ...
    while (reader.has_next()) {
        ...
        if (bag_msg->topic_name != "/ublox/fix") continue;
...
int main()
{
    ...
    std::vector<sensor_msgs::msg::NavSatFix::Ptr> navsat_msgs;
    ...
    for (int i; i < navsat_msgs.size(); i++){
        if (navsat_msgs.at(i)->position_covariance.at(0) > 3 || navsat_msgs.at(i)->position_covariance.at(8) > 8)
                continue;
        geographic_msgs::msg::GeoPointStampedPtr gps_msg(new geographic_msgs::msg::GeoPointStamped());
        ...
        utm_stamps.push_back(rclcpp::Time(navsat_msgs.at(i)->header.stamp).seconds());
    }

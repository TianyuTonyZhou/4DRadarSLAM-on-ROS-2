...
LoopDetector::LoopDetector(rclcpp::Node::SharedPtr node) {
    enable_pf              = (node->has_parameter("enable_pf") ? node->get_parameter("enable_pf").get_value<bool>() : node->declare_parameter<bool>("enable_pf", true));
    enable_odom_check      = (node->has_parameter("enable_odom_check") ? node->get_parameter("enable_odom_check").get_value<bool>() : node->declare_parameter<bool>("enable_odom_check", true));

    distance_thresh        = (node->has_parameter("distance_thresh") ? node->get_parameter("distance_thresh").get_value<double>() : node->declare_parameter<double>("distance_thresh", 5.0));
    accum_distance_thresh  = (node->has_parameter("accum_distance_thresh") ? node->get_parameter("accum_distance_thresh").get_value<double>() : node->declare_parameter<double>("accum_distance_thresh", 8.0));
    min_loop_interval_dist = (node->has_parameter("min_loop_interval_dist") ? node->get_parameter("min_loop_interval_dist").get_value<double>() : node->declare_parameter<double>("min_loop_interval_dist", 5.0));
    
    fitness_score_max_range= (node->has_parameter("fitness_score_max_range") ? node->get_parameter("fitness_score_max_range").get_value<double>() : node->declare_parameter<double>("fitness_score_max_range", std::numeric_limits<double>::max()));
    fitness_score_thresh   = (node->has_parameter("fitness_score_thresh") ? node->get_parameter("fitness_score_thresh").get_value<double>() : node->declare_parameter<double>("fitness_score_thresh", 0.5));

    odom_drift_xy          = (node->has_parameter("odometry_drift_xy") ? node->get_parameter("odometry_drift_xy").get_value<double>() : node->declare_parameter<double>("odometry_drift_xy", 0.05));
    odom_drift_z           = (node->has_parameter("odometry_drift_z") ? node->get_parameter("odometry_drift_z").get_value<double>() : node->declare_parameter<double>("odometry_drift_z", 0.20));
    drift_scale_xy         = (node->has_parameter("odometry_drift_scale_xy") ? node->get_parameter("odometry_drift_scale_xy").get_value<double>() : node->declare_parameter<double>("odometry_drift_scale_xy", 2.0));
    drift_scale_z          = (node->has_parameter("odometry_drift_scale_z") ? node->get_parameter("odometry_drift_scale_z").get_value<double>() : node->declare_parameter<double>("odometry_drift_scale_z", 2.0));

    max_baro_difference    = (node->has_parameter("max_baro_difference") ? node->get_parameter("max_baro_difference").get_value<double>() : node->declare_parameter<double>("max_baro_difference", 3.0));
    max_yaw_difference     = (node->has_parameter("max_yaw_difference") ? node->get_parameter("max_yaw_difference").get_value<double>() : node->declare_parameter<double>("max_yaw_difference", 45.0));

    double sc_dist_thresh  = (node->has_parameter("sc_dist_thresh") ? node->get_parameter("sc_dist_thresh").get_value<double>() : node->declare_parameter<double>("sc_dist_thresh", 0.3));
    double sc_azimuth_range= (node->has_parameter("sc_azimuth_range") ? node->get_parameter("sc_azimuth_range").get_value<double>() : node->declare_parameter<double>("sc_azimuth_range", 0.3));

    registration           = select_registration_method(node);

    // Loop Closure Parameters
    historyKeyframeSearchNum    = (node->has_parameter("historyKeyframeSearchNum") ? node->get_parameter("historyKeyframeSearchNum").get_value<int>() : node->declare_parameter<int>("historyKeyframeSearchNum", 25));
    historyKeyframeFitnessScore = (node->has_parameter("historyKeyframeFitnessScore") ? node->get_parameter("historyKeyframeFitnessScore").get_value<double>() : node->declare_parameter<double>("historyKeyframeFitnessScore", 6.0));
    odom_check_trans_thresh  = (node->has_parameter("odom_check_trans_thresh") ? node->get_parameter("odom_check_trans_thresh").get_value<double>() : node->declare_parameter<double>("odom_check_trans_thresh", 0.1));
    odom_check_rot_thresh    = (node->has_parameter("odom_check_rot_thresh") ? node->get_parameter("odom_check_rot_thresh").get_value<double>() : node->declare_parameter<double>("odom_check_rot_thresh", 0.05));
    pairwise_check_trans_thresh = (node->has_parameter("pairwise_check_trans_thresh") ? node->get_parameter("pairwise_check_trans_thresh").get_value<double>() : node->declare_parameter<double>("pairwise_check_trans_thresh", 0.1));
    pairwise_check_rot_thresh   = (node->has_parameter("pairwise_check_rot_thresh") ? node->get_parameter("pairwise_check_rot_thresh").get_value<double>() : node->declare_parameter<double>("pairwise_check_rot_thresh", 0.05));
...
Loop::Ptr LoopDetector::performScanContextLoopClosure(const std::vector<KeyFrame::Ptr>& keyframes, const std::vector<KeyFrame::Ptr>& candidate_keyframes, const KeyFrame::Ptr& new_keyframe)
{
    ...
    if (!loopIndexQueue.empty()){
        for (size_t i = loopIndexQueue.size()-1; i < loopIndexQueue.size(); i++) {
            ...
            std::cout << "\033[31m Pairwise check invalid, Translation Error: " << pcc_err_trans << ", rotation Error: " << pcc_err_rot << " \033[0m" << std::endl;
            return nullptr;
            }
        }
    }
    ...
    sensor_msgs::msg::Image::SharedPtr cur_img_msg = cv_bridge::CvImage(std_msgs::msg::Header(), "bgr8", color_cur_sc).toImageMsg();
    pub_cur_sc.publish(cur_img_msg);
    sensor_msgs::msg::Image::SharedPtr pre_img_msg = cv_bridge::CvImage(std_msgs::msg::Header(), "bgr8", color_pre_sc).toImageMsg();
...
Loop::Ptr LoopDetector::matching(const std::vector<KeyFrame::Ptr>& candidate_keyframes, const KeyFrame::Ptr& new_keyframe) {
    ...
    std::cout << "best_score: " << boost::format("%.3f") % best_score << "    time: " << boost::format("%.3f") % elapsed << "[sec]" << std::endl;

                                         ...
                                         sensor_msgs::msg::PointCloud2& outlier_radar_msg)
{
  ...
  if (1) // pcl2msgToPcl(radar_scan_msg, *radar_scan)
  {
    ...
    else std::cout << "To small valid_targets (< 2) in radar_scan (" << radar_scan->size() << ")" << std::endl;
                                                  ...
                                                  std::vector<uint>& outlier_idx_best)
{
  ...
  if (radar_data.rows() >= config_.N_ransac_points)
  {
    for (uint k = 0; k < ransac_iter_; ++k)
    {
      ...
      if (rtn == false) std::cout << "Failure at solve3DFullRansac() 1" << std::endl;
      if (rtn)
      {
  ...
  else{std::cout << "Warning: radar_data.rows() < config_.N_ransac_points" << std::endl;}
  ...
  if (!inlier_idx_best.empty())
  {
    ...
    if (rtn == false) std::cout << "Failure at solve3DFullRansac() 2" << std::endl;
    return rtn;
  }
  std::cout << "Failure at solve3DFullRansac() 3" << std::endl;
  return false;
}

...
class ScanMatchingOdometryComponent : public rclcpp::Node, public ParamServer {
public:
  typedef pcl::PointXYZI PointT;

  using TwistMsg = geometry_msgs::msg::TwistWithCovarianceStamped;
  using CloudMsg = sensor_msgs::msg::PointCloud2;
  using ApproxSyncPolicy = message_filters::sync_policies::ApproximateTime<TwistMsg, CloudMsg>;
  ...
  explicit ScanMatchingOdometryComponent(const rclcpp::NodeOptions & options)
  : Node("scan_matching_odometry", options), ParamServer(this)
  {
    ...
    if(declare_parameter<bool>("enable_imu_frontend", false)) {
      msf_pose_sub = create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(
        ...
        [this](geometry_msgs::msg::PoseWithCovarianceStamped::ConstSharedPtr msg){
          msf_pose_callback(msg, false);
        });
      msf_pose_after_update_sub = create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(
        "/msf_core/pose_after_update", 
        1, 
        [this](geometry_msgs::msg::PoseWithCovarianceStamped::ConstSharedPtr msg){
          msf_pose_callback(msg, true);
        });
    }
    //******** Subscribers **********
    ego_vel_sub = std::make_shared<message_filters::Subscriber<TwistMsg>>(
      this,
      "/eagle_data/twist"
    );
    points_sub = std::make_shared<message_filters::Subscriber<CloudMsg>>(
      this,
      "/filtered_points"
    );
    sync = std::make_shared<message_filters::Synchronizer<ApproxSyncPolicy>>(
      ApproxSyncPolicy(32),
      *ego_vel_sub,
      *points_sub
    );
    ...
    imu_sub = create_subscription<sensor_msgs::msg::Imu>(
      "/imu",
      1024,
      std::bind(&ScanMatchingOdometryComponent::imu_callback, 
        this,
        std::placeholders::_1)
    );
    ...
    command_sub = create_subscription<std_msgs::msg::String>(
      "/command",
      10,
      std::bind(&ScanMatchingOdometryComponent::command_callback,
        this,
        std::placeholders::_1)
    );

    //******** Publishers **********
    read_until_pub = create_publisher<std_msgs::msg::Header>(
      "/scan_matching_odometry/read_until", 
      32
    );
    ...
    trans_pub = create_publisher<geometry_msgs::msg::TransformStamped>(
      "/scan_matching_odometry/transform", 
      32
    );
    status_pub = create_publisher<radar_graph_slam::msg::ScanMatchingStatus>(
      "/scan_matching_odometry/status", 
      8
    );
    aligned_points_pub = create_publisher<sensor_msgs::msg::PointCloud2>(
      "/aligned_points", 
      32
    );
    submap_pub = create_publisher<sensor_msgs::msg::PointCloud2>(
      "/radar_graph_slam/submap", 
      2
    );
    map2odom_broadcaster = std::make_shared<tf2_ros::TransformBroadcaster>(*this);
    ...
    use_ego_vel = declare_parameter<bool>("use_ego_vel", false);
    ...
    enable_transform_thresholding = declare_parameter<bool>("enable_transform_thresholding", false);
    enable_imu_thresholding = declare_parameter<bool>("enable_imu_thresholding", false);
    ...
    max_submap_frames = declare_parameter<int>("max_submap_frames", 5);

    enable_imu_fusion = declare_parameter<bool>("enable_imu_fusion", false);
    ...
    init_timer_ = create_wall_timer(
      ...
      [this]() {
        ...
        registration_s2s = select_registration_method(shared_from_this());
        registration_s2m = select_registration_method(shared_from_this());
      }
    );
  }
...
private:
  ...
  void initialize_params() {
    ...
    // graph_slam.reset(new GraphSLAM(declare_parameter<std::string>("g2o_solver_type", "lm_var")));
  ...
  void imu_callback(const sensor_msgs::msg::Imu::ConstSharedPtr imu_msg) {
    ...
    imuTime[imuPointerLast] = rclcpp::Time(imu_msg->header.stamp).seconds();
    ...
    static int cnt = 0;
    if(cnt == 0) {
      tf2::Quaternion imuQuat_tf2;
      imuQuat_tf2.setRPY(roll, pitch, 0.0);
      global_orient_matrix = Eigen::Quaterniond(
        imuQuat_tf2.w(), 
        imuQuat_tf2.x(), 
        imuQuat_tf2.y(), 
        imuQuat_tf2.z()).toRotationMatrix();

      RCLCPP_INFO_STREAM(get_logger(), "Initial IMU euler angles (RPY): "
  ...
  bool flush_imu_queue() {
    ...
    auto remove_loc = std::upper_bound(imu_queue.begin(), imu_queue.end(), keyframes.back()->stamp, [=](const rclcpp::Time& stamp, const sensor_msgs::msg::Imu::ConstSharedPtr& imupoint) { return stamp < rclcpp::Time(imupoint->header.stamp); });
    
    imu_queue.erase(imu_queue.begin(), remove_loc);
    return updated;
  }

  std::pair<bool, sensor_msgs::msg::Imu> get_closest_imu(rclcpp::Time frame_stamp) {
    sensor_msgs::msg::Imu imu_;
    std::pair<bool, sensor_msgs::msg::Imu> false_result {false, imu_};
    if(keyframes.empty() || imu_queue.empty())
      return false_result;
    bool updated = false;
    auto imu_cursor = imu_queue.begin();
    ...
    auto closest_imu = imu_cursor;
    for(auto imu = imu_cursor; imu != imu_queue.end(); imu++) {
      auto dt  = (rclcpp::Time((*closest_imu)->header.stamp) - frame_stamp).seconds();
      auto dt2 = (rclcpp::Time((*imu)->header.stamp) - frame_stamp).seconds();
      if(std::abs(dt) < std::abs(dt2)) {
        break;
      }
      closest_imu = imu;
    ...
    // if the time residual between the imu and keyframe_ is too large, skip it
    imu_cursor = closest_imu;
    if(0.2 < std::abs((rclcpp::Time((*closest_imu)->header.stamp) - frame_stamp).seconds()))
      return false_result;
    ...
    std::pair<bool, sensor_msgs::msg::Imu> result {updated, imu_};
    return result;
  }
  ...
  void transformUpdate(Eigen::Matrix4d& odom_to_update) // IMU
  {
		if (imuPointerLast >= 0) 
    {
      ...
      tf2::Quaternion imuQuat_tf2; imuQuat_tf2.setRPY(imuRollLast, imuPitchLast, ypr_odom(0));
      Eigen::Matrix3d imu_rot = Eigen::Matrix3d(Eigen::Quaterniond(imuQuat_tf2.w(), imuQuat_tf2.x(), imuQuat_tf2.y(), imuQuat_tf2.z())); 
      ...
      tf2::Quaternion rosQuat_tf2; rosQuat_tf2.setRPY(roll_fused, pitch_fused, yaw_);
      Eigen::Quaterniond quat_updated(rosQuat_tf2.w(), rosQuat_tf2.x(), rosQuat_tf2.y(), rosQuat_tf2.z());
   ...
   */
  void pointcloud_callback(TwistMsg::ConstSharedPtr twistMsg, CloudMsg::ConstSharedPtr cloud_msg) {
    if(!rclcpp::ok()) {
      return;
    }
    timeLaserOdometry = rclcpp::Time(cloud_msg->header.stamp).seconds();
    double this_cloud_time = rclcpp::Time(cloud_msg->header.stamp).seconds();
    ...
    geometry_msgs::msg::TwistWithCovariance twist = twistMsg->twist;
  ...
  Eigen::Matrix4d matching(const rclcpp::Time& stamp, const pcl::PointCloud<PointT>::ConstPtr& cloud) {
    ...
    if(!registration_s2s->hasConverged()) {
      RCLCPP_INFO_STREAM(get_logger(), "scan matching_ has not converged!!");
    ...
    if(enable_transform_thresholding) {
      ...
      if (enable_imu_thresholding) {
        ...
        if (dx > max_diff_trans || da > max_diff_angle || too_large_trans) {
          ...
          cout << "Difference of Odom and IMU/EgoVel too large " << dx << "[m] " << da << "[deg] (" << stamp.seconds() << ")" << endl;
    ...
    if(keyframe_updater->decide(Eigen::Isometry3d(odom_s2s_now), stamp)) {
      ...
      if (enable_scan_to_map){
        ...
        submap_ros.header.stamp = stamp;
        submap_pub->publish(submap_ros);
    ...
    if (aligned_points_pub->get_subscription_count() > 0)
    {
      ...
      pcl::toROSMsg(*aligned, aligned_ros);
      aligned_ros.header.frame_id = odometryFrame;
      aligned_points_pub->publish(aligned_ros);
    }
   ...
   */
  void publish_odometry(const rclcpp::Time& stamp, const std::string& father_frame_id, const std::string& child_frame_id, const Eigen::Matrix4d& pose_in, const geometry_msgs::msg::TwistWithCovariance twist_in) {
    // publish transform stamped for IMU integration
    geometry_msgs::msg::TransformStamped odom_trans = matrix2transform(stamp, pose_in, father_frame_id, child_frame_id); //"map" 
   ...
   */
  void publish_scan_matching_status(const rclcpp::Time& stamp, const std::string& frame_id, pcl::PointCloud<pcl::PointXYZI>::ConstPtr aligned, const std::string& msf_source, const Eigen::Isometry3d& msf_delta) {
    if(status_pub->get_subscription_count() == 0) {
      return;
    }

    radar_graph_slam::msg::ScanMatchingStatus status;
  ...
  void command_callback(std_msgs::msg::String::ConstSharedPtr str_msg) {
...
private:
  ...
  std::shared_ptr<message_filters::Subscriber<TwistMsg>> ego_vel_sub;
  std::shared_ptr<message_filters::Subscriber<CloudMsg>> points_sub;
  std::shared_ptr<message_filters::Synchronizer<ApproxSyncPolicy>> sync;
  std::mutex imu_queue_mutex;

  sensor_msgs::msg::Imu last_frame_imu;
  ...
  rclcpp::Publisher<radar_graph_slam::msg::ScanMatchingStatus>::SharedPtr status_pub;
  rclcpp::Publisher<std_msgs::msg::Header>::SharedPtr read_until_pub;
  std::shared_ptr<tf2_ros::TransformBroadcaster> map2odom_broadcaster;  
  rclcpp::Time prev_time;
  rclcpp::Time keyframe_stamp;
...
RCLCPP_COMPONENTS_REGISTER_NODE(radar_graph_slam::ScanMatchingOdometryComponent)
